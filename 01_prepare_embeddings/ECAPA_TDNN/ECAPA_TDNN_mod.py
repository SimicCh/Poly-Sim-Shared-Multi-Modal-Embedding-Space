...
class ECAPA_TDNN(torch.nn.Module):
    ...
    def __init__(
        ...
        groups=[1, 1, 1, 1, 1],
        dropout=0.0,
        finetune_strategy='no_layer',   # 'no_layer', 'last_layer', 'all_layers'
    ):
        super().__init__()
        ...
        # Finetuning strategy
        if finetune_strategy == 'no_layer':
            for p in self.parameters():
                p.requires_grad = False
        elif finetune_strategy == 'last_layer':
            for p in self.parameters():
                p.requires_grad = False
            for p in self.fc.parameters():
                p.requires_grad = True
        elif finetune_strategy == 'all_layers':
            for p in self.parameters():
                p.requires_grad = True
        else:
            raise ValueError(f"Unknown finetune strategy: {finetune_strategy}")
    ...
    def forward_prefc(self, x, lengths=None):
        """Returns the embedding vector.
